 ...
 */
namespace common\addons\giiant\generators\model;
...
class Generator extends \yii\gii\generators\model\Generator
{
     ...
     * @var bool whether or not to use BlameableBehavior
     */
    public $useSlugableBehavior = true;
    ...
    public $slugCollumn = 'slug';
    public $useThumbnailBehaviour = true;
    public $thumbnailColumnName = ['base_url','base_path'];

    public $useAttachmentBehaviour = false;

    public $useImagesBehaviour = false;
    public $imagesTableName = '{{table}}_images';
    public $imagesRelColumn = 'master_id';
    ...
    {
        return 'Генератор полной модели';
    }
    ...
    {
        return 'Этот генератор создает модель под текущую сборку Yii2StarterKitClear 18.01.2018';
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if(count($pieces) > 1){
                $preparedName = implode('_',[ucfirst($pieces[0]),ucfirst($pieces[1])]);
            }else{
                $preparedName = ucfirst($pieces[0]);
            }

           $classNameForSearch = str_replace('_','',$preparedName.'Attachments');

           if(!is_null($relations[$tableName][$classNameForSearch])){
               $this->useAttachmentBehaviour = true;
           }else{
               $this->useAttachmentBehaviour = false;
           }
...
//var_dump($relations,$tableName);exit;
            ...
            $params['thumbnail'] = $this->generateThumbnail($tableSchema);
            $params['attachments'] = $this->useAttachmentBehaviour;
            $params['slug'] = $this->generateSlug($tableSchema);
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $index => $column) {
            ...
            $isThumbnailUrl = ($column->name === $this->thumbnailColumnName[0]);
            $isSlugColumn = ($column->name === $this->slugCollumn);
            $removeCol =
                ($this->useBlameableBehavior && $isBlameableCol)
                ||
                ($this->useTimestampBehavior && $isTimestampCol || $this->useThumbnailBehaviour && $isThumbnailUrl)
                ||
                ($this->useSlugableBehavior && $isSlugColumn);
            if ($removeCol) {
     ...
     * @return string[]
     */
    protected function generateSlug($table)
    {
        $slug = $table->getColumn($this->slugCollumn) !== null ? $this->slugCollumn : false;

        if ($this->useSlugableBehavior && $slug) {
            return $slug;
        }
    ...
    protected function generateThumbnail($table)
    {
        $thumbnail_url = $table->getColumn($this->thumbnailColumnName[0]) !== null ? $this->thumbnailColumnName[0] : false;

        if ($this->useThumbnailBehaviour && ($thumbnail_url)) {
            return [
